"""
Module définissant l'architecture CNN de base pour la classification d'écorces d'arbres.
"""

import torch
import torch.nn as nn
import torch.nn.functional as F

from ..config import NUM_CLASSES


class BaseCNN(nn.Module):
    """
    Architecture CNN de base pour la classification d'écorces d'arbres.
    
    Cette architecture comprend:
    - 4 blocs de convolution avec activation ReLU et max pooling
    - 2 couches fully connected avec dropout
    """
    
    def __init__(self, num_classes=NUM_CLASSES):
        """
        Initialisation du modèle.
        
        Args:
            num_classes (int): Nombre de classes (espèces d'arbres) à classifier
        """
        super(BaseCNN, self).__init__()
        
        # Bloc 1 de convolution
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(32)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Bloc 2 de convolution
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(64)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Bloc 3 de convolution
        self.conv3 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.bn3 = nn.BatchNorm2d(128)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Bloc 4 de convolution
        self.conv4 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.bn4 = nn.BatchNorm2d(256)
        self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        # Couches fully connected
        # Calcul de la taille d'entrée pour la première couche fully connected
        # Avec une image d'entrée de 224x224, après 4 max pooling avec stride 2,
        # la taille est réduite à 14x14
        self.fc1 = nn.Linear(256 * 14 * 14, 512)
        self.drop1 = nn.Dropout(0.5)
        self.fc2 = nn.Linear(512, num_classes)
    
    def forward(self, x):
        """
        Passe avant du modèle.
        
        Args:
            x (torch.Tensor): Batch d'images d'entrée de forme [batch_size, 3, height, width]
            
        Returns:
            torch.Tensor: Prédictions de classes de forme [batch_size, num_classes]
        """
        # Bloc 1
        x = self.pool1(F.relu(self.bn1(self.conv1(x))))
        
        # Bloc 2
        x = self.pool2(F.relu(self.bn2(self.conv2(x))))
        
        # Bloc 3
        x = self.pool3(F.relu(self.bn3(self.conv3(x))))
        
        # Bloc 4
        x = self.pool4(F.relu(self.bn4(self.conv4(x))))
        
        # Aplatir le tenseur
        x = x.view(x.size(0), -1)
        
        # Couches fully connected
        x = F.relu(self.fc1(x))
        x = self.drop1(x)
        x = self.fc2(x)
        
        return x
    
    def predict(self, x):
        """
        Prédit les classes pour un batch d'images.
        
        Args:
            x (torch.Tensor): Batch d'images d'entrée
            
        Returns:
            torch.Tensor: Classes prédites
        """
        self.eval()
        with torch.no_grad():
            outputs = self(x)
            _, predicted = torch.max(outputs, 1)
        return predicted 